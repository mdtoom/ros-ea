...
class UnfilteredStatesGatherer:
    """ This class receives all sim_reports and writes them to a file, no filtering is applied."""

    def __init__(self, name_space, base_dir):
        stripped_ns = name_space.replace('/', "")
        self.states_file = base_dir + "unfiltered_sim_report_{0}.csv".format(stripped_ns)
        rospy.Subscriber(name_space + 'simreport_topic', SimulationReport, self.callback)
    ...
    def callback(self, data):
        ...
        data_array = [data.header.generation, data.header.key, data.score, data.at_light] + state_usage

        # Write the data to the data_file.
        with open(self.states_file, 'a') as csv_file:
            writer = csv.writer(csv_file)
